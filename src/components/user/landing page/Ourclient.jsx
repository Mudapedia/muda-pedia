  ...
  return (
    <div className='flex flex-col max-w-screen-lg mx-auto w-full gap-10'>
      <div className='flex max-w-screen-lg mx-auto w-full gap-16 justify-between'>
        ...
        <div className='flex lg:w-1/2 md:w-full w-full lg:px-0 px-6 flex-col gap-3 items-start'>
          ...
          <p className='text-gray-500'>
            At Mudapedia, we take immense pride in our diverse and esteemed clientele. Our clients span various industries and sectors, each benefiting from our innovative solutions and dedicated service. We believe in building long-term relationships based on trust, transparency, and mutual growth. Here are some of the valued clients who have chosen Mudapedia as their trusted partner:
          </p>
          <button className=' text-[#4F5CDF] rounded-md w-full justify-start flex gap-3' onClick={() => window.open('https://api.whatapps.com/message/62895621048269')}>
            Lihat Semua Mitra Kami <svg width="24px" height="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M4 12H20M20 12L14 6M20 12L14 18" stroke="#4F5CDF" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>
          </button>
        </div>
      </div>
      <div className='lg:block hidden max-w-screen-lg mx-auto'>
        ...
        >
          {data.map((item, index) => (
            <SwiperSlide key={index} className=' w-full flex flex-col items-start lg:h-52 bg-gray-100 rounded-xl py-4 px-5 gap-12 cursor-pointer' onClick={()=>{
      ...
      <div className='lg:hidden block max-w-screen-lg mx-auto px-6'>
        <div className='grid w-full gap-5 grid-cols-2'>
          {data.map((item, index) => {
            return (
              <div className=' w-full flex flex-col items-start lg:h-52 bg-gray-100 rounded-xl py-4 px-5 gap-1 cursor-pointer' onClick={()=>{
                window.open(item.link)
              }}>
                <img src={item.logo} alt="" width={50} height={50} />
              <p className='font-semibold'>{item.name}</p>
