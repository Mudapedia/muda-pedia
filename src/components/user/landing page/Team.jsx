...
const Team = () => {
  ...
  return (
    <div className='max-w-screen-lg mx-auto w-full flex flex-col justify-between gap-16 items-center lg:px-0 px-6 scroll-mt-[8vh\' id='team'>
        ...
        <div className='flex justify-center items-center' key={index}>
          <div className='justify-between flex items-center flex-col gap-3'>
            ...
            <div className='flex flex-col w-full items-center'>
              ...
              <p className='text-md  text-[#66676A] text-center'>{item.position}</p>
            </div>
          </div>
        </div>
      ))}
      <div className="grid lg:grid-cols-2 grid-cols-2 text-center lg:gap-0 gap-5 w-full place-items-center">
        {mudapediaTeam.slice(1, 3).map((item, index) => (
          <div className='justify-between flex items-center flex-col gap-3' key={index}>
            <Image src={item.image} className='rounded-full w-28' />
            <div className='flex flex-col w-full items-center'>
              <h4 className='text-lg font-medium'>{item.name}</h4>
              <p className='text-md  text-[#66676A] text-center'>{item.position}</p>
      ...
      <div className="flex flex-wrap justify-center max-w-screen-md gap-10 items-center">
        {pavoTeam.map((item, index) => (
          <div className='justify-between flex items-center flex-col gap-3 w-[45%]' key={index}>
            <Image src={item.image} className='rounded-full w-28' />
            <div className='flex flex-col w-full items-center'>
              <h4 className='text-lg font-medium'>{item.name}</h4>
              <p className='text-md  text-[#66676A] text-center'>{item.position}</p>
